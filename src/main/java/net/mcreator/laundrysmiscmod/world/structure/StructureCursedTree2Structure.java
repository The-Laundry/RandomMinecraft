...
public class StructureCursedTree2Structure extends LaundrysMiscModElements.ModElement {
	public StructureCursedTree2Structure(LaundrysMiscModElements instance) {
		super(instance, 120);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("laundrysmiscmod", "cursed_tree_2"));
						if (template == null)
							return false;
